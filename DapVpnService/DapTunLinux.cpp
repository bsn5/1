...
{
    qDebug() << "[DapTunLinux::onWorkerStarted]";
    QProcess process;
    ...
    if(m_defaultGwOld.isEmpty()){
        ...
        m_defaultGwOld.chop(1);
        if(m_defaultGwOld.isEmpty()){
            qWarning() << "[SaCHSF] Not found old gateway, looks like its better to restart the network";
            emit error(tr("No gateway in system") );
            return;
        }
...
//    ::system((QString("nmcli connection modify DiveVPN"
//        " +ipv4.routes \"%1/32 %2 10\"").arg(SaSession::me()->address()).arg(m_defaultGwOld))
//        .toLatin1().constData());
